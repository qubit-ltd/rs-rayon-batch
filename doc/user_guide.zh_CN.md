...
Rayon 线程池中时，执行器会选择顺序路径。最后一种情况是重入保护：任务可以调用同一
执行器的 clone，而不会等待被父任务占用的 worker。
...
Rayon worker；如果调用已经在同一线程池中，仍会走顺序回退。
...
同步调用契约，并避免嵌套线程池死锁。嵌套调用的结果不会自动合并到外层结果；任务应按
应用的错误模型检查或转换这个结果。
...
| 任务调用了同一个执行器 | 任务是否运行在该执行器线程池中 | 嵌套调用会按设计顺序回退；在任务内检查它的结果。 |
...
- 同池重入会回退到顺序执行，不会额外制造并行度，也不会自动合并嵌套结果。
